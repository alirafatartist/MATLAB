function installaddon(zip_file)
%
% INSTALLADDON Install the specified addon to the current MATLAB installation.
%   INSTALLDDON ZIP_FILE.ZIP Install the contents of ZIP_FILE to MATLABROOT.

%   Copyright 2008-2024 The MathWorks, Inc.

% first check if the jvm is available
if (~usejava('jvm'))
    error('installaddon requires Java to run.')
end

% check args
if (nargin ~= 1)
    error('Usage: installaddon <addon_zip_file>')
end
